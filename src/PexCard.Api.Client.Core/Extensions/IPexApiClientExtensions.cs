...
namespace PexCard.Api.Client.Core.Extensions
{
    public static class IPexApiClientExtensions
    {
        ...
        public static async Task<Dictionary<int, List<AllocationTagValue>>> GetTagAllocations(
            this IPexApiClient pexApiClient,
            string externalToken,
            List<BillPaymentRequestModel> billPaymentRequests,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            if (pexApiClient is null)
            {
            ...
            if (billPaymentRequests is null)
            {
                throw new ArgumentNullException(nameof(billPaymentRequests));
            }

            var tagDefinitions = new List<TagDetailsModel>();
            if (billPaymentRequests.Any(t => t.Metadata?.TagAnswers != null))
            {
                //Only get the tag definitions if we see tags on the bill payment requests.
                //If a businesses uses tags for some time then disables them, this will throw an exception.
                tagDefinitions.AddRange(await pexApiClient.GetTags(externalToken, cancellationToken));
            }
        ...
        public static Task<Dictionary<int, List<AllocationTagValue>>> GetTagAllocations(
            this IPexApiClient pexApiClient,
            string externalToken,
            List<BillPaymentRequestModel> billPaymentRequests,
            List<TagDetailsModel> tagDefinitions)
        {
            ...
            var result = new Dictionary<int, List<AllocationTagValue>>();

            var hashedTagDefinitions = tagDefinitions.ToDictionary(key => key.Id, value => value);

            foreach (var billPaymentRequest in billPaymentRequests)
            {
                if (result.ContainsKey(billPaymentRequest.PaymentRequestId))
                {
                    //Skip bill payment requests that have already been processed
                    //Assumption: If a bill payment request is passed in multiple times each one will have the same tags
                    continue;
                }

                var allocations = new List<AllocationTagValue>();

                var defaultAllocation = new AllocationTagValue { TagId = null, Allocation = new List<TagValueItem>(), Amount = Math.Abs(billPaymentRequest.Amount), };
                if (billPaymentRequest.Metadata?.TagAnswers != null)
                {
                    foreach (var tag in billPaymentRequest.Metadata.TagAnswers)
                    {
                        if (!hashedTagDefinitions.TryGetValue(tag.FieldId, out var tagDefinition))
                        {
                            throw new KeyNotFoundException($"Unable to find tag definition for {nameof(TagDetailsModel.Id)} '{tag.FieldId}'.");
                ...
                if (!allocations.Any())
                {
                    //Ensure each bill payment request has at least one allocation
                    allocations.Add(defaultAllocation);
                }

                result.Add(billPaymentRequest.PaymentRequestId, allocations);
            }
